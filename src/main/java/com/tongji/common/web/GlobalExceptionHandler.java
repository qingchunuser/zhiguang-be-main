...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleOss(OSSException ex) {
        log.error("OSS异常: errorCode={}, errorMessage={}", ex.getErrorCode(), ex.getMessage(), ex);
        Map<String, Object> body = new HashMap<>();
        body.put("code", ex.getErrorCode());
        body.put("message", "文件存储服务异常: " + ex.getMessage());
        if ("AccessDenied".equals(ex.getErrorCode())) {
            body.put("message", "OSS访问权限不足，无法完成操作，请联系管理员配置正确的RAM权限");
        }
