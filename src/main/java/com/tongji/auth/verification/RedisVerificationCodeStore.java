...
public class RedisVerificationCodeStore implements VerificationCodeStore {
    ...
    public VerificationCheckResult verify(String scene, String identifier, String code) {
        ...
        String storedCode = data.get(FIELD_CODE);//这是生成的验证码（原来的验证码）
        int maxAttempts = parseInt(data.get(FIELD_MAX_ATTEMPTS), 5);//如果data.get为空 返回值5，不是则data.get(FIELD_MAX_ATTEMPTS)
        ...
        ops.put(key, FIELD_ATTEMPTS, String.valueOf(updatedAttempts));//修改验证码次数,更新尝试次数，保存到redis中
