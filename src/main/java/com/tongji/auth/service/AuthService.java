...
public class AuthService {
    ...
    public SendCodeResponse sendCode(SendCodeRequest request) {
        validateIdentifier(request.identifierType(), request.identifier());//校验手机号或邮箱格式
        String normalized = normalizeIdentifier(request.identifierType(), request.identifier());//规范化手机号或邮箱格式
        boolean exists = identifierExists(request.identifierType(), normalized);//检查手机号或邮箱是否在数据库中存在
        if (request.scene() == VerificationScene.REGISTER && exists) {//用已经注册过的手机号或邮箱进行注册
    ...
    public AuthResponse register(RegisterRequest request, ClientInfo clientInfo) {
        ...
        if (identifierExists(request.identifierType(), identifier)) {//用手机号或邮箱查询用户是否注册
            throw new BusinessException(ErrorCode.IDENTIFIER_EXISTS);
        }
        ensureVerificationSuccess(verificationService.verify(VerificationScene.REGISTER, identifier, request.code()));//确保验证码校验成功
        ...
        if (StringUtils.hasText(request.password())) {
            ...
            user.setPasswordHash(passwordEncoder.encode(request.password().trim()));//encode() 方法会对密码进行哈希加密处理
        }

        userService.createUser(user);//添加数据库
        TokenPair tokenPair = jwtService.issueTokenPair(user);//生成JWT令牌跟他的过期时间
        storeRefreshToken(user.getId(), tokenPair);//存入redis中
    ...
    public AuthResponse login(LoginRequest request, ClientInfo clientInfo) {
        validateIdentifier(request.identifierType(), request.identifier());//校验手机号或邮箱格式
        String identifier = normalizeIdentifier(request.identifierType(), request.identifier());//规范化手机号或邮箱格式
        Optional<User> userOptional = findUserByIdentifier(request.identifierType(), identifier);//查询用户
        ...
        if (StringUtils.hasText(request.password())) {
            ...
            channel = "CODE";
            VerificationCheckResult verify = verificationService.verify(VerificationScene.LOGIN, identifier, request.code());//验证码校验
            ensureVerificationSuccess(verify);//确保验证码校验成功,VerificationCodeStatus类的状态进行分析
        } else {
    ...
    private String normalizeIdentifier(IdentifierType type, String identifier) {
        return switch (type) {
            case PHONE -> identifier.trim();/*当账号类型是手机号时，仅去除字符串两端的空白字符*/
            case EMAIL -> identifier.trim().toLowerCase(Locale.ROOT);/*当账号类型是邮箱时，不仅去除两端空白字符，还将邮箱地址转换为小写形式*/
        };
    }
