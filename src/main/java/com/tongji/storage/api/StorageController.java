    ...
    public StoragePresignResponse presign(@Valid @RequestBody StoragePresignRequest request,
        ...
        if ("knowpost_content".equals(scene)) {
            ...
            /*UUID.randomUUID() - 生成一个随机的UUID
             .toString() - 将UUID转换为字符串格式（形如：a1b2c3d4-e5f6-7890-abcd-ef1234567890）
             .replaceAll("-", "") - 移除所有的连字符"-"
             * .substring(0, 8) - 截取前8个字符*/
            String rand = UUID.randomUUID().toString().replaceAll("-", "").substring(0, 8);
            objectKey = "posts/" + postId + "/images/" + date + "/" + rand + ext;
        } else {
            log.info("不支持的上传场景 {}", scene);
        ...
        log.info("presign putUrl {} objectKey {} headers {} expiresIn {}", putUrl, objectKey, headers, expiresIn);
