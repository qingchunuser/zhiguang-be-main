...
public class OssStorageService {
    ...
    public String uploadAvatar(long userId, MultipartFile file) {
        ...
        try {
            ...
            log.info("头像上传成功: bucket={}, key={}, userId={}", props.getBucket(), objectKey, userId);
        } catch (OSSException e) {
            log.error("OSS上传失败: bucket={}, key={}, errorCode={}, errorMessage={}", 
                     props.getBucket(), objectKey, e.getErrorCode(), e.getMessage(), e);
            if ("AccessDenied".equals(e.getErrorCode())) {
                throw new BusinessException(ErrorCode.BAD_REQUEST, "OSS访问权限不足，请联系管理员配置权限");
            }
            throw new BusinessException(ErrorCode.BAD_REQUEST, "文件上传失败: " + e.getMessage());
        } catch (IOException e) {
            log.error("头像文件读取失败: userId={}, fileName={}", userId, original, e);
