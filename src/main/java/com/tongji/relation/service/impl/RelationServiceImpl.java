...
public class RelationServiceImpl implements RelationService {
    ...
    private static final String TOKEN_BUCKET_LUA = "token-bucket.lua";
    ...
    public RelationServiceImpl(RelationMapper mapper,
        ...
        this.tokenScript.setLocation(new ClassPathResource(TOKEN_BUCKET_LUA));
    ...
    public boolean follow(long fromUserId, long toUserId) {
        // Lua 脚本令牌桶限流
        //lua脚本内容（hash结构）：
                // 用key来访问，key：last，key：tokens
                // 当进行第一次获取令牌时候会先初始化为令牌桶数为100，时间设为当前时间的时间戳
                // 当前时间减去上一次访问的时间乘上1（一秒内生成一个令牌，最多不能超过100）
                // 如果令牌数小于1，说明令牌已经用问了，返回0
                // 最后令牌足够的话删除一个令牌，然后进行保存，返回1
        ...
        int inserted = mapper.insertFollowing(id, fromUserId, toUserId, 1);//写入数据库

        if (inserted > 0) {
            try {
                Long outId = ThreadLocalRandom.current().nextLong(Long.MAX_VALUE);//随机id
    ...
    public boolean unfollow(long fromUserId, long toUserId) {
        int updated = mapper.cancelFollowing(fromUserId, toUserId);//把rel_status=0变成零，关注的关系断开
        if (updated > 0) {
            try {
    ...
    public Map<String, Boolean> relationStatus(long userId, long otherUserId) {
        boolean following = isFollowing(userId, otherUserId);//当前用户是否关注对方用户
        boolean followedBy = isFollowing(otherUserId, userId);//对方用户是否关注当前用户
    ...
    public List<ProfileResponse> followingProfiles(long userId, int limit, int offset, Long cursor) {
        List<Long> ids =  cursor != null ? followingCursor(userId, limit, cursor)//向下无限滑动
                : following(userId, limit, offset);//跳转页面看 逻辑为：查redis查到返回，没查到查本地缓存，查到返回没查到查数据库，数据库存到redis，redis存本地缓存，最后返回
    ...
    private List<ProfileResponse> toProfiles(List<Long> ids) {
        ...
        Map<Long, User> m = new HashMap<>(users.size());
     ...
     *
     * 查redis查到返回，没查到查本地缓存，查到返回没查到查数据库，数据库存到redis，redis存本地缓存，最后返回
     */
    private List<Long> getListWithOffset(
        ...
        Set<String> cached = redis.opsForZSet().reverseRange(key, offset, offset + limit - 1L);//返回关注数
        ...
        if (rows != null && !rows.isEmpty()) {
            ...
            if (localCache != null && isBigV(userId)) {
                maybeUpdateTopCache(userId, key, localCache); // ← 存储到本地缓存
            }
    ...
    private List<Long> getListWithCursor(String key,
        ...
        Set<String> cached = redis.opsForZSet().reverseRangeByScore(key, max, Double.NEGATIVE_INFINITY, 0, limit);//reverseRangeByScore倒序排，从max到无穷小,第0开始查limit数据
        if (cached != null && !cached.isEmpty()) {
            return toLongList(cached);
        }
     ...
     * 456: { toUserId: 456, createdAt: 1709800000000 },
     * 789: { toUserId: 789, createdAt: 1709799000000 },
     * 101: { toUserId: 101, createdAt: 1709798000000 }
     */
    private void fillZSet(String key,
        ...
        for (Map<String, Object> r : rows.values()) {
            ...
            Object tsObj = r.get(tsField);//createdAt
