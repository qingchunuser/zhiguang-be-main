    ...
    public void start() {
        ...
        taskExecutor.execute(() -> {
            try {
                ...
                log.info("canal的连接中 {}:{} dest={} user={} filter={}", host, port, destination, username, filter);
                connector.connect();
                // 订阅过滤表达式，仅拉取关心的表（如 outbox）
                connector.subscribe(filter);
                // 回滚到上次确认位点，保证一致性处理
                connector.rollback();
                //batchSize:每次拉取数据的最大条数,intervalMs:拉取数据的时间间隔(毫秒)
                log.info("Canal 已连接并订阅：主机={} 端口={} 目标={} 过滤器={} 批次大小={} 轮询间隔={}ms", host, port, destination, filter, batchSize, intervalMs);
                while (running) {
                    // 拉取一批未确认消息（不自动 ack）
                    ...
                    for (CanalEntry.Entry entry : message.getEntries()) {
                        ...
                        try {
                            ...
                            kafka.send(OutboxTopics.CANAL_OUTBOX, json);//创建了kafka的topic：canal-outbox
