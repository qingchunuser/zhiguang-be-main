...
public class ActionController {
    ...
    public ResponseEntity<Map<String, Object>> like(@Valid @RequestBody ActionRequest req,
        ...
        boolean changed = counterService.like(req.getEntityType(), req.getEntityId(), uid);//在redis跟kafka中写入
        return ResponseEntity.ok(Map.of(
                "changed", changed, // 标识这次操作是否改变状态（避免重复点击）
                "liked", counterService.isLiked(req.getEntityType(), req.getEntityId(), uid)//判断是否点赞
        ));
    }
