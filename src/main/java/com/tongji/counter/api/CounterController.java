...
public class  CounterController {
    ...
    public ResponseEntity<CountsResponse> getCounts(@PathVariable("etype") String entityType,
                                                    ...
                                                    //metrics 是一个可选的查询参数，用于过滤返回的计数指标。多个指标之间用逗号分隔。
                                                    @RequestParam(value = "metrics", required = false) String metricsStr) {
        List<String> metrics;
        if (metricsStr == null || metricsStr.isBlank()) {
            metrics = new ArrayList<>(CounterSchema.SUPPORTED_METRICS); // 未指定指标时返回全部支持的计数,like,fiv
        } else {
            metrics = Arrays.stream(metricsStr.split(","))
                    .map(String::trim)//map() - 转换元素
                    .filter(CounterSchema.SUPPORTED_METRICS::contains) // 过滤未知指标，保证请求安全,.filter(元素 -> true/false)
                    .toList();
        }
