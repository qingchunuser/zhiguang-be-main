...
public class CounterServiceImpl implements CounterService {
    ...
    private static final String TOGGLE_LUA_SCRIPT_PATH = "toggle-bitmap.lua";
    ...
    public CounterServiceImpl(StringRedisTemplate redis, CounterEventProducer eventProducer, ApplicationEventPublisher eventPublisher, RedissonClient redisson) {
        ...
        this.toggleScript.setLocation(new ClassPathResource(TOGGLE_LUA_SCRIPT_PATH));
    }
    ...
    private boolean toggle(String etype, String eid, long uid, String metric, int idx, boolean add) {
        ...
        //从0到32_768都为0
        long chunk = BitmapShard.chunkOf(uid);// public static final int CHUNK_SIZE = 32_768;   userId / CHUNK_SIZE;
        // 分片内位偏移
        //
...
/*
        ...
        从上下文来看，这段代码是在执行一个Redis Lua脚本，用于原子性地切换位图状态。让我分析一下：

        1. `toggleScript` 是一个Lua脚本，用于原子性地设置或清除位图中的特定位
        2. `keys` 包含位图的Redis键
        3. `args` 包含位图偏移量和操作类型("add"或"remove")
        4. 返回值`changed`表示是否有状态变化(1表示有变化，其他值表示无变化)

        根据业务逻辑，这个操作是用来处理点赞/收藏等状态切换的。我可以重写这段代码，使其更加清晰和健壮：*/
        Long changed = redis.execute(toggleScript, keys, args.toArray());// 执行Lua脚本，切换位图状态
        if (changed == null) {
            // 处理Redis执行异常的情况
            throw new RuntimeException("Failed to execute toggle operation for " + metric + " on entity " + etype + ":" + eid);
        }
        boolean ok = changed == 1L;//说明已经存储成功
        if (ok) {
            int delta = add ? 1 : -1;
    ...
    public Map<String, Long> getCounts(String entityType, String entityId, List<String> metrics) {
        ...
        // SDS 固定结构：按大端 32 位编码
        byte[] raw = getRaw(sdsKey);//二进制数据
        boolean needRebuild = (raw == null || raw.length != expectedLen);//二进制数据，共二十个字节，判断二进制是否是二十个字节或者为null
        ...
        if (needRebuild) {
            ...
            try {
                ...
                for (String m : metrics) {
                    ...
                    long sum = bitCountShardsPipelined(m, entityType, entityId);//用bitcount来算出数据
     ...
     */
    @Override
    //"knowpost", needCountsIds, List.of("like", "fav")
    public Map<String, Map<String, Long>> getCountsBatch(String entityType, List<String> entityIds, List<String> metrics) {
        ...
        for (String eid : entityIds) {
            keys.add(CounterKeys.sdsKey(entityType, eid));//变成SDS的key
        }
        ...
        for (int i = 0; i < entityIds.size(); i++) {
            ...
            if (raw != null && raw.length == expectedLen) {
                for (String name : metrics) {
                    ...
                    if (idx == null) continue;
                    int off = idx * CounterSchema.FIELD_SIZE;
                    long val = readInt32BE(raw, off);//读取点赞或者收藏数据
                    m.put(name, val);
                }
            } else {
                for (String name : metrics) {
     ...
     * String eKey = String.format("backoff:sds-rebuild:exp:%s:%s", entityType, entityId);//退避级别
     * String uKey = String.format("backoff:sds-rebuild:until:%s:%s", entityType, entityId);//截止时间戳
     */
    private boolean inBackoff(String entityType, String entityId) {
        // bKey 的格式示例："backoff:sds-rebuild:until:knowpost:123"
        String bKey = String.format("backoff:sds-rebuild:until:%s:%s", entityType, entityId);
        // 从 Redis 获取这个 key 对应的值
    ...
    private boolean allowedByRateLimiter(String entityType, String entityId) {
        ...
        // 初始化速率（如已存在则忽略），每 10 秒内最多允许 3 次重建操作
    ...
    private long bitCountShardsPipelined(String metric, String etype, String eid) {
        //每个用户（chunk）中存的key， String.format("bm:%s:%s:%s:%d", metric, entityType, entityId, chunk)
